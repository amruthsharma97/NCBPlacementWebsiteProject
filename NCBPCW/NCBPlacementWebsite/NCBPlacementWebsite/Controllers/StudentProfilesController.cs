...
namespace NCBPlacementWebsite.Controllers
{
    ...
    public class StudentProfilesController : Controller
    {
        ...
        public ActionResult Index(int? page,string branch)
        {
            ...
            orderbystatus=studbranlist.OrderBy(s => s.Status).ThenBy(s=>s.FName).ToList();
        ...
        public ActionResult Create(ApplicationUser user)
        {
            ...
            Dictionary<int, string> ddictionary = new Dictionary<int, string>();

            for (int i = 1; i <=31; i++)
            {
                ddictionary.Add(i, i.ToString());
            }

            ViewBag.dt = new SelectList(ddictionary, "Key", "Value");
            ...
            for (int i = 1; i <= 12; i++)
            {
                mdictionary.Add(i, i.ToString());
            }

            ViewBag.mth = new SelectList(mdictionary, "Key", "Value");
        ...
        {
            studentProfile.DOB = String.Join("-", studentProfile.date.ToString(), studentProfile.month.ToString(), studentProfile.year.ToString());
            if (ModelState.IsValid)
            {
                if (studentProfile.SSLCYOP >= studentProfile.PUCYOP || studentProfile.SSLCYOP >= studentProfile.DegreeYOP || studentProfile.SSLCYOP >= studentProfile.PUCYOP && studentProfile.SSLCYOP >= studentProfile.DegreeYOP || studentProfile.PUCYOP - studentProfile.SSLCYOP < 2)
                {
        ...
        public void CreateExcel(int? yop,string branch,float? min10perc, float? minpuperc )
        {
            ...
            if (ExcelSP.Count!=0)
            {
                ...
                foreach (var item in ExcelSP)
                {
                    ...
                    ws.Cells[string.Format("J{0}", rowStart)].Value = item.AddrLine1;
                    ws.Cells[string.Format("K{0}", rowStart)].Value = item.AddrLine2;
                    ws.Cells[string.Format("L{0}", rowStart)].Value = item.City;
                    ws.Cells[string.Format("M{0}", rowStart)].Value = item.PostalCode;
                    ws.Cells[string.Format("N{0}", rowStart)].Value = item.State;
                    ws.Cells[string.Format("O{0}", rowStart)].Value = item.Country;
                    ws.Cells[string.Format("P{0}", rowStart)].Value = item.SSLCPercentage;
                    ws.Cells[string.Format("Q{0}", rowStart)].Value = item.SSLCBoard;
                    ws.Cells[string.Format("R{0}", rowStart)].Value = item.NameOfSchool;
                    ws.Cells[string.Format("S{0}", rowStart)].Value = item.SSLCPassingState;
                    ws.Cells[string.Format("T{0}", rowStart)].Value = item.SSLCPassingCOuntry;
                    ws.Cells[string.Format("U{0}", rowStart)].Value = item.SSLCYOP;
                    ws.Cells[string.Format("V{0}", rowStart)].Value = item.PUCPercentage;
                    ws.Cells[string.Format("W{0}", rowStart)].Value = item.PUCBoard;
                    ws.Cells[string.Format("X{0}", rowStart)].Value = item.NameOfClg;
                    ws.Cells[string.Format("Y{0}", rowStart)].Value = item.PUCPassingState;
                    ws.Cells[string.Format("Z{0}", rowStart)].Value = item.PUCPassingCOuntry;
                    ws.Cells[string.Format("AA{0}", rowStart)].Value = item.PUCYOP;
                    ws.Cells[string.Format("AB{0}", rowStart)].Value = item.DegreePercentage;
                    ws.Cells[string.Format("AC{0}", rowStart)].Value = item.Branch;
                    ws.Cells[string.Format("AD{0}", rowStart)].Value = "Bangalore University";
                    ws.Cells[string.Format("AE{0}", rowStart)].Value = "The National College, Basavanagudi";
                    ws.Cells[string.Format("AF{0}", rowStart)].Value = "Karnataka";
