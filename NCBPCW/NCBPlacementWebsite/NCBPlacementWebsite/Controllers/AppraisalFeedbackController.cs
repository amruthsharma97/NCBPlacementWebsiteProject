...
namespace NCBPlacementWebsite.Controllers
{
    public class AppraisalFeedbackController : Controller
    {
        ...
        [Authorize(Roles = "Admin")]
        public ActionResult RemoveAppraisalFeedback()
        {
            int yr = 1994;
        ...
        [Authorize(Roles = "Admin")]
        [HttpPost]
        public ActionResult RemoveAppraisalFeedback(RemoveFeedback removeFeedback)
        {
            List<AppraisalFeedbacks> afbyyear = new List<AppraisalFeedbacks>();



            afbyyear = db.AppraisalFeedback.Where(a => a.EYear == removeFeedback.Year).ToList();

            if (ModelState.IsValid && afbyyear.Count() > 0)
            {
                foreach (var item in afbyyear)
                {
                    db.AppraisalFeedback.Remove(item);
                    db.SaveChanges();
                }
